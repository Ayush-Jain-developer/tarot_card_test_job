...
class ValidateFields {
    ...
    static integerRequired(field, word, min, max) {
        const schema = joi_1.default.number().integer().min(min).max(max).required();
        const validation = schema.validate(field);
        if (validation.error) {
            const message = (0, _helper_1.errorMessage)(word, validation.error.details[0].message);
            throw new _exceptions_1.BadRequestExceptionError(message);
    ...
    static decimalRequired(field, word) {
        const decimalSchema = joi_1.default.number()
            .precision(2)
            .min(0)
            .max(9999999.99)
            .required();
