...
class UserController {
    static async signUp(req, res, next) {
        const data = req.body;
        try {
            const response = await _service_1.UserService.signUp(req, data);
    ...
    static async getUser(req, res, next) {
        const userId = req.body.id;
        try {
            const response = await _service_1.UserService.getUser(userId);
    ...
    static async getAllReaders(req, res, next) {
        ...
        try {
            const response = await _service_1.UserService.getAllReaders(Number(pageNumber), Number(pageSize));
