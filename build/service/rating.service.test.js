...
describe("Rating and review API", () => {
    ...
    test("should return error for rating already given", async () => {
        _repo_1.RatingRepo.findRating = jest
            .fn()
            .mockResolvedValue({ dataValues: mockRatingData });
        try {
            await _service_1.RatingService.createRating(mockRatingData);
            expect(_repo_1.RatingRepo.findRating).toBeCalledWith(mockRatingData.senderId, mockRatingData.receiverId);
        }
        catch (error) {
            expect(error).toBeInstanceOf(_exceptions_1.BadRequestExceptionError);
            expect(error.message).toBe(_messages_1.default.ratingGiven);
        }
    });
    test("should return rating data", async () => {
        _repo_1.RatingRepo.findRating = jest.fn().mockResolvedValue({ dataValues: false });
        _repo_1.RatingRepo.createRating = jest
            .fn()
            .mockResolvedValue({ dataValues: mockRatingData });
        const response = await _service_1.RatingService.createRating(mockRatingData);
        expect(_repo_1.RatingRepo.findRating).toBeCalled();
        expect(_repo_1.RatingRepo.createRating).toBeCalled();
